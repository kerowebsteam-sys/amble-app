...
const DEFAULT_PLAYBOOK_TASKS = [
  "Signed management agreement received",
  "Property photos taken",
  "Listing title & description written",
  "Amenities list completed",
  "House rules & guest info guide created",
  "Pricing set (comps logged, base rate agreed with owner)",
  "Cleaning/turnover schedule arranged",
  "Instant Book enabled",
  "Calendar synced across Airbnb + Booking.com",
  "Listing published and live",
  "Owner welcome pack sent",
];
...
const NAV = [
  ...
  { id: "playbook", label: "Listing playbook", icon: ListChecks },
...
export default function AmbleApp() {
  ...
  async function loadAll() {
    ...
    const [p, c, b, s, d, f, l, pb] = await Promise.all([
      ...
      supabase.from("playbook_tasks").select("*").order("sort_order"),
    ]);
            ...
            <>
              ...
              {tab === "playbook" && (
                <Playbook
                  ...
                  setPlaybookTasks={setPlaybookTasks}
                  userId={userId}
                  showToast={showToast}
                />
              )}
          ...
          <Card>
            ...
            <div className="space-y-3">
              ...
              <Field
                label={
                  mode === "upload"
                    ? "Name / description (optional if uploading multiple)"
                    : "Name / description"
                }
              >
                <input
              ...
              {mode === "upload" ? (
                <Field
                  label={`File(s)${pendingFiles.length ? ` — ${pendingFiles.length} selected` : ""}`}
                >
                  <input
                    type="file"
                    multiple
...
function Playbook({
  ...
  const tasks = playbookTasks
    .filter((t) => t.property_id === selectedId)
    .sort((a, b) => a.sort_order - b.sort_order);
  const doneCount = tasks.filter((t) => t.completed).length;
  const pct = tasks.length ? Math.round((doneCount / tasks.length) * 100) : 0;

  async function loadStarterChecklist() {
    const rows = DEFAULT_PLAYBOOK_TASKS.map((task, i) => ({
      property_id: selectedId,
      user_id: userId,
      task,
      completed: false,
    ...
    setPlaybookTasks([...playbookTasks, ...data]);
    showToast("Starter checklist loaded");
  }
  ...
  async function toggleTask(t) {
    const { data, error } = await supabase
      ...
      .single();
    if (error) return;
    setPlaybookTasks(playbookTasks.map((x) => (x.id === t.id ? data : x)));
  }

  async function removeTask(id) {
    await supabase.from("playbook_tasks").delete().eq("id", id);
    setPlaybookTasks(playbookTasks.filter((t) => t.id !== id));
  }

  async function move(t, direction) {
    ...
    setPlaybookTasks(
      playbookTasks.map((x) =>
        x.id === r1.data.id ? r1.data : x.id === r2.data.id ? r2.data : x,
      ),
    );
  }
  ...
  return (
    <div>
      <h1 className="amble-display text-3xl mb-6">Listing playbook</h1>
      <select
        value={selectedId}
        onChange={(e) => setSelectedId(e.target.value)}
        className="text-sm px-3 py-2 rounded-md border mb-6"
      ...
      {properties.length === 0 ? (
        ...
        <Card>
          <p className="text-sm mb-3" style={{ color: COLORS.warmgray }}>
            No checklist yet for this property.
          </p>
          <button
            onClick={loadStarterChecklist}
        ...
        <>
          ...
          <Card>
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      ...
                      color: t.completed ? COLORS.warmgray : COLORS.charcoal,
                    }}
                  >
                    {t.task}
                  </span>
                  ...
                  <button
                    ...
                    style={{
                      color: COLORS.warmgray,
                      opacity: i === tasks.length - 1 ? 0.3 : 1,
                    }}
                  >
                    <ArrowDown size={13} />
                  </button>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={addTask}
                className="text-sm px-4 py-2 rounded-md flex items-center gap-1"
                style={{ background: COLORS.teal, color: COLORS.offwhite }}
              >
                <Plus size={14} />
                Add
